...
def select_best_room(
    ...
    """Phase D — room scoring formula for next-room selection.

    Scores each known room and returns the highest-scoring one:

        room_score(r) = w1*prior(r) + w2*evidence(r) + w3*unexplored(r)
                      - w4*cost(r) - w5*penalty(r)

    Parameters
    ----------
    search_state    : SearchState — contains one RoomState per room.
    kept_components : heatmap components annotated with "room" and "quality".
    robot_pos       : (row, col) current robot position on the VLMap grid.
    exclude_rooms   : room names to skip (already exhausted this search).
    w1..w5          : scoring weights (defaults from the PDF spec).

    Returns
    -------
    (best_room_name, scores_dict)
    best_room_name is None if no room is available after exclusion.
    scores_dict maps room_name → raw score for every non-excluded room.
    """
    if not search_state or not search_state.rooms:
        return None, {}

    if exclude_rooms is None:
        exclude_rooms = set()

    # ── evidence(r): sum of heatmap component quality for each room ──────
    raw_evidence: dict = {}
    for comp in kept_components:
        room = comp.get("room")
        if room:
            raw_evidence[room] = raw_evidence.get(room, 0.0) + float(comp.get("quality", 0.0))
    max_ev = max(raw_evidence.values(), default=0.0)
    if max_ev > 1e-6:
        ev_norm = {r: v / max_ev for r, v in raw_evidence.items()}
    else:
        ev_norm = {}

    # ── cost(r): Euclidean distance robot → room centroid, normalised ────
    rr, rc = float(robot_pos[0]), float(robot_pos[1])
    distances: dict = {}
    for name, rs in search_state.rooms.items():
        if name in exclude_rooms:
            continue
        dr = rs.centroid[0] - rr
        dc = rs.centroid[1] - rc
        distances[name] = float(np.sqrt(dr * dr + dc * dc))
    max_dist = max(distances.values(), default=1.0)
    if max_dist < 1e-6:
        max_dist = 1.0

    # ── penalty(r): normalised failed-candidate count ─────────────────────
    max_tried = max(
        (rs.candidates_tried for rs in search_state.rooms.values()), default=0
    )

    # ── Score each non-excluded room ──────────────────────────────────────
    scores: dict = {}
    for name, rs in search_state.rooms.items():
        if name in exclude_rooms:
            continue

        prior      = float(rs.target_relevance)
        evidence   = float(ev_norm.get(name, 0.0))
        unexplored = 1.0 - float(rs.explored_ratio)
        cost       = distances.get(name, 0.0) / max_dist
        penalty    = float(rs.candidates_tried) / max(max_tried, 1)
    ...
    return best_room, scores


def filter_candidates_by_room(kept_components: list, room_name: str) -> list:
    """Phase E — return only heatmap components whose centroid is in room_name."""
    return [c for c in kept_components if c.get("room") == room_name]
...
def main(config: DictConfig) -> None:
    ...
    while True:
        ...
        for cat in categories:
            ...
            # ── Phase D+E: room-switching loop ────────────────────────────
            # Activates when Stages 0-3 all failed and room information is
            # available.  Uses the Phase D scoring formula to select the
            # next-best room, navigates there, then searches Phase-E-filtered
            # candidates within it (up to _MAX_ROOM_SWITCHES iterations).
            if (not _yoloe_confirmed and room_goal is None
                    and _ss and _room_provider
                    and _room_provider.is_available()
                    and kept_components):

                robot._set_nav_curr_pose()
                _pd_current = _room_provider.get_room_at_cell(
                    int(robot.curr_pos_on_map[0]), int(robot.curr_pos_on_map[1])
                )
                _pd_exhausted: set = set()
                if _pd_current:
                    _pd_exhausted.add(_pd_current)

                _MAX_ROOM_SWITCHES = 3

                for _pd_switch in range(_MAX_ROOM_SWITCHES):
                    if _yoloe_confirmed:
                        break

                    robot._set_nav_curr_pose()
                    _pd_best, _pd_scores = select_best_room(
                        _ss, kept_components, robot.curr_pos_on_map,
                        exclude_rooms=_pd_exhausted,
                    )
                    if _pd_best is None:
                        print(f"  [Phase D] No more rooms to explore — stopping.")
                        break

                    _pd_top = ", ".join(
                        f"{r}={v:.3f}"
                        for r, v in sorted(_pd_scores.items(), key=lambda x: -x[1])[:6]
                    )
                    print(f"  [Phase D] Switch {_pd_switch+1}/{_MAX_ROOM_SWITCHES} "
                          f"— scores: {_pd_top}")
                    print(f"  [Phase D] Selected room: '{_pd_best}'")

                    # Navigate to the selected room
                    _pd_room_goals = find_reachable_room_goal(
                        _pd_best, _room_provider, _safe_map
                    )
                    if not _pd_room_goals:
                        print(f"  [Phase D] Room '{_pd_best}' unreachable — skipping.")
                        _pd_exhausted.add(_pd_best)
                        continue

                    _pd_goal = _pd_room_goals[0]
                    _, _pd_room_acts = robot.plan_path_only(_pd_goal)
                    _pd_room_path = getattr(robot, "last_planned_path", None) or []

                    if len(_pd_room_acts) > 0:
                        show_map(robot, rgb_map_2d, heatmap_2d=heatmap,
                                 path_cells=_pd_room_path,
                                 label=f"Phase D \u2192 {_pd_best}")
                        cv2.waitKey(800)
                        print(f"  [Phase D] Navigating to '{_pd_best}' "
                              f"({len(_pd_room_acts)} actions)…")
                        execute_nav_replay(
                            robot, _pd_room_acts, cat,
                            rgb_map_2d, heatmap, _pd_room_path,
                            dist_map=_dist_map_shield,
                        )
                    else:
                        print(f"  [Phase D] Already near '{_pd_best}'.")

                    robot._set_nav_curr_pose()
                    _ss.update_current_room(_pd_best)

                    # Phase E: filter candidates to the selected room
                    _pe_all     = filter_candidates_by_room(kept_components, _pd_best)
                    _pe_untried = [
                        c for c in _pe_all
                        if (int(c["centroid"][0]), int(c["centroid"][1]))
                           not in _ss._tried_centroids
                    ]
                    print(f"  [Phase E] Room '{_pd_best}': "
                          f"{len(_pe_all)} component(s) total, "
                          f"{len(_pe_untried)} untried.")

                    if not _pe_untried:
                        print(f"  [Phase E] No untried candidates in "
                              f"'{_pd_best}' — marking exhausted.")
                        _pd_exhausted.add(_pd_best)
                        continue

                    # Try untried candidates in this room (up to 3)
                    for _pe_comp in _pe_untried[:3]:
                        if _yoloe_confirmed:
                            break

                        _pe_cen = [int(_pe_comp["centroid"][0]),
                                   int(_pe_comp["centroid"][1])]
                        print(f"  [Phase E] Candidate {_pe_cen}  "
                              f"quality={_pe_comp.get('quality', 0):.3f}")

                        # Approach goal around candidate centroid
                        robot._set_nav_curr_pose()
                        _pe_app = find_safe_candidate_approach_goals(
                            _pe_cen, _safe_map,
                            robot_pos=getattr(robot, "_nav_curr_pos", None),
                            min_dist=8.0, max_dist=25.0, min_clearance=3.0,
                        )
                        _pe_goal = _pe_app[0] if _pe_app else _pe_cen

                        _, _pe_acts = robot.plan_path_only(_pe_goal)
                        _pe_path = getattr(robot, "last_planned_path", None) or []

                        if len(_pe_acts) > 0:
                            show_map(robot, rgb_map_2d, heatmap_2d=heatmap,
                                     path_cells=_pe_path,
                                     label=f"Phase E: {cat} in {_pd_best}")
                            cv2.waitKey(400)
                            execute_nav_replay(
                                robot, _pe_acts, cat,
                                rgb_map_2d, heatmap, _pe_path,
                                dist_map=_dist_map_shield,
                            )

                        robot._set_nav_curr_pose()
                        face_toward_pos(robot, _pe_cen[0], _pe_cen[1])
                        show_obs(robot, f"Phase E: {cat} in {_pd_best}")

                        # YOLOE verification at this candidate
                        _pe_confirmed = False
                        if _yoloe_session is not None:
                            try:
                                _pe_obs = robot.sim.get_sensor_observations(0)
                                if "color_sensor" in _pe_obs:
                                    _pe_frame = _pe_obs["color_sensor"][:, :, :3]
                                    _pe_confirmed, _pe_ann, _pe_bbox = (
                                        _yoloe_session.check(_pe_frame)
                                    )
                                    if _pe_ann is not None:
                                        _pe_bgr = cv2.cvtColor(
                                            _pe_ann, cv2.COLOR_RGB2BGR
                                        )
                                        show_obs(robot,
                                                 f"Phase E YOLOE: {cat}",
                                                 yoloe_frame_bgr=_pe_bgr)
                                    if _pe_confirmed:
                                        print(f"  [Phase E] \u2713 Found '{cat}' "
                                              f"in '{_pd_best}'!")
                                        if _pe_ann is not None:
                                            _frozen_detection_bgr = cv2.cvtColor(
                                                _pe_ann, cv2.COLOR_RGB2BGR
                                            )
                                        fine_visual_center(
                                            robot, _yoloe_session, cat
                                        )
                                    else:
                                        print(f"  [Phase E] \u2717 '{cat}' not "
                                              f"found at {_pe_cen}.")
                            except Exception as _pe_exc:
                                print(f"  [Phase E] YOLOE error: {_pe_exc}")

                        # Record outcome in SearchState
                        _pe_actual = _room_provider.get_room_at_cell(
                            int(robot.curr_pos_on_map[0]),
                            int(robot.curr_pos_on_map[1]),
                        )
                        _ss.record_candidate(_pe_actual, _pe_confirmed,
                                             centroid=_pe_cen)
                        if _pe_confirmed:
                            _yoloe_confirmed = True
                            _ss.record_object_seen(_pe_actual, cat)
                            _ss.mark_found(_pe_actual)
